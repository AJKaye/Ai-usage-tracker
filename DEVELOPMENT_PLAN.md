...
**Last updated:** 2026-08-06 · **Overall status:** **Phases 0–10 complete for the self-contained tier** — GA-ready as a downloadable single-file `.exe` with the full pipeline (ingest → normalize → cost → reconcile → allocate → serve → UI → MCP → export), 172 tests green under `-warnaserror`, live-verified end-to-end. **Self-contained now includes columnar OLAP** via the embedded **DuckDB `analytics` profile** (ADR-0010) — so the analytics capability D1's ClickHouse was for ships with zero infra. The **distributed/SaaS stack (ClickHouse/Postgres/Kafka/Helm/K8s, load+chaos, pen test) is reframed as the multi-node horizontal-scale / HA option** — an evidence-triggered upgrade, not a missing capability — with contracts + conformance suites + runbooks in place. See per-phase `[x]`/`[~]`/`[ ]` status + ADR-0010.
...
- [x] **Deployment profiles + config-driven selection** at the composition root (`solo`/`analytics`/`ephemeral`/`standard`/`distributed`) — the one-line backend swap, ADR-0009/0010.
- [x] **Self-contained single-file publish** (win/linux/macOS × x64/arm64) wired into CI (`publish-exe` job); win-x64 exe built + run here.
- [x] **`IEventStore` → embedded columnar DuckDB** (`UsageTracker.Storage.DuckDb`, `analytics` profile) — the self-contained **OLAP** backend: in-engine columnar SUM/GROUP BY, still zero-infra (native lib in the `.exe`). **Passes the full `EventStoreContractTests`**; live-verified on the exe (columnar summary + survives restart). **This closes the analytics-capability half of D1 without a server** (ADR-0010).
- [~] `IEventStore` → **ClickHouse** — *multi-node-HA scale option only (ADR-0010), not a capability gap: `analytics`/DuckDB delivers columnar OLAP self-contained.* Contract + conformance suite ready; no impl yet (infra-blocked — needs a cluster). Build + CI-verify with service containers if an evidence trigger (§ below) or owner requirement calls for it.
- [~] `IRelationalStore` → **Postgres** — *multi-node relational/HA option only.* Contract declared (`StorageAndStream.cs`); catalog/tenancy state is served by the embedded stores today. No Npgsql impl yet (infra-blocked).
- [~] `IStreamBus` → **Kafka** — *multi-producer/at-volume durable-queue option only.* The in-process `IIngestChannel` (bounded, backpressure, idempotent `(tenant,span)`) is the shipping self-contained equivalent; Kafka is the horizontal-scale slot-in (infra-blocked).
- [x] Migration tooling — `DataPortabilityService` + `/v1/export`→`/v1/import` (Phase 10) is the store-agnostic migration substrate (proven cross-store: SQLite↔DuckDB↔InMemory).
- [x] Golden-dataset harness — the canonical fixtures live in `EventStoreContractTests` (every store impl — InMemory, SQLite, **DuckDB** — runs them).

> **Reframed 2026-08-06 (ADR-0010):** the self-contained tier now covers **both** halves of what D1 was for — durable relational state (`solo`/SQLite), **columnar analytics** (`analytics`/DuckDB), and a backpressured durable queue (`IIngestChannel`) — all in one zero-infra `.exe`, all passing the same conformance suite. The ClickHouse/Postgres/Kafka backends are **no longer a missing capability**; they are the **multi-node horizontal-scale / HA** option, to be reached for only on an **evidence trigger** — sustained ingest that pins a single node (watch `/v1/platform/stats` `queueDepth` climbing and never draining) or a hard availability SLA. Because everything sits behind the same contracts, that switch is a **config change + data export/import**, not a redesign.
...
- [x] **Audit:** **tamper-evident `HashChainAuditSink`** (hash chain, `Verify` detects mutation/reorder, tenant-scoped, exportable evidence) — **now enforced in the request path**: middleware records an `AuditEvent` on every mutating (`POST`/`PUT`/`DELETE`) `/v1`+`/mcp` request; `GET /v1/audit` exports the chain + integrity check. Live-verified (ingest+content+delete → 3-entry intact chain).
- [x] **Data lifecycle:** per-subject **crypto-shredding** right-to-delete — **now a live request path**: `POST /v1/content` (opt-in capture, sealed per-subject; 409 unless enabled), `GET /v1/content/{span}`, `DELETE /v1/subjects/{id}` (crypto-shred). Live-verified: capture→reveal→erase→404 while the cost aggregate persists. `DataLifecyclePolicy` gates capture (off by default). *ClickHouse-partition crypto-shred is the same mechanism at the scale tier.*
- [x] **Air-gap validation:** `EgressPolicy` (now the `IEgressGuard` contract) is **enforced AT the outbound call sites** — `HttpCatalogSource` + both billing connectors `AssertEgressAllowed` before any HTTP, so air-gap fails closed at the site (not merely by omission); tests prove the outbound handler is never reached under `solo`. The signed offline pricing bundle is **enforced when `USAGETRACKER__PRICING_*` is configured** (verified before load; tampered → refused).
...
> **Phase-7 remainder (infra-blocked or additive):** Postgres RLS + ClickHouse row policies; mTLS + TLS 1.3 + KMS-wrapped DEKs + FIPS; OIDC/SAML SSO + SCIM; per-endpoint RBAC filter + per-tenant rate limiting; SBOM/scan/signing in CI; third-party pen test (Phase 10). *(Audit emission, air-gap egress enforcement, and the crypto-shred content path are now wired into the request path — 2026-08-06.)*
